...
1. Vite runs in **watch mode**, rebuilding whenever you change files
2. Files are automatically written to `dist/` folder
3. Open `about:debugging#/runtime/this-firefox`
4. Click **Load Temporary Add-on…** → select `dist/manifest.json`
5. When you see build output (e.g., `built in XXms`), reload the extension in Firefox
6. Your changes are now live!

**Note:** No HMR for Firefox (MV2 limitation), but watch mode rebuilds are instant.

### Production Builds

```bash
npm run build        # Chrome MV3 → dist/manifest.json (MV3)
npm run build:firefox # Firefox MV2 → dist/manifest.json (MV2)
```
...
- **Firefox**: Uses custom `firefoxPlugin` to write manifest and copy CSS (no CRXJS needed for MV2)
